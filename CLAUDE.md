...
Single-package React Native app (not a monorepo) using Yarn 1.22.22 (npm won't work). Supports iOS 13.4+ and Android 6.0+.
...
yarn android               # Build and run on Android (uses experimentalDebug mode)
yarn android:production    # Production Android build (experimentalRelease mode)
...
# E2E (Maestro)
yarn e2e:start             # Start Metro for E2E tests (sets RUNNING_E2E_TESTS, enabling .mock.ts overrides)
